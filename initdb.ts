...
import { MealsType } from "./components/MealsGrid";
const db = sql("meals.db");

export const dummyMeals: MealsType[] = [
  {
...
export function initDb() {
  ...
  const row = db.prepare("SELECT COUNT(*) as c FROM meals").get() as { c: number };
  if (row.c === 0) {
    ...
    insert.run({
      ...
      slug: "pizza",
      image: "/pizza.jpg",
      summary: "Classic pizza",
      instructions: "Bake and enjoy.",
      creator: "Chef",
